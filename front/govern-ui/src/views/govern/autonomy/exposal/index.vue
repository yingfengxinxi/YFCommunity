...
<!--        <el-form-item label="所属小区d" prop="userCommunityName">-->
<!--          <el-input-->
...
<!--        </el-form-item>-->
        ...
        <el-form-item label="所属小区" prop="userCommunityName">
          <el-select v-model="queryParams.userCommunityName"  placeholder="请选择小区" style="width: 100%">
            <el-option
      ...
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="multiple"
            @click="handleDelete"
            v-hasPermi="['autonomy:exposal:delete']"
          >删除
          </el-button>
        </el-col>
        ...
        >
          <template slot-scope="scope">
            <el-button
              ...
              v-hasPermi="['autonomy:exposal:query']"
            >查看
            </el-button>
            <el-button
              size="mini"
              ...
              style="color: #f56c6c"
              icon="el-icon-delete"
              @click="handleDeleteMsg(scope.row)"
              v-hasPermi="['autonomy:exposal:delete']"
            >删除
            </el-button>
          </template>
    ...
    <!--系统信息-->
    <div v-show=checkSysView>
      <!--模糊查询-->
      <el-form :model="querySysParams" ref="sysQueryForm" size="small" :inline="true" v-show="sysShowSearch">
        <el-form-item label="发布日期">
          <el-date-picker
            v-model="dateRange"
            style="width: 240px"
        ...
        <el-form-item>
          <el-button type="primary" icon="el-icon-search" size="mini" @click="sysHandleQuery">搜 索</el-button>
          <el-button icon="el-icon-refresh" size="mini" @click="sysResetQuery">重 置</el-button>
        </el-form-item>
      </el-form>
      <!--按钮操作-->
      <el-row :gutter="10" class="mb8">
        ...
        <el-col :span="1.5">
          <el-button
            ...
            size="mini"
            :disabled="multiple"
            @click="sysHandleDelete"
            v-hasPermi="['autonomy:exposal:delete']"
          >删除
          </el-button>
        </el-col>
        <right-toolbar :showSearch.sync="sysShowSearch" @queryTable="getList"></right-toolbar>
      </el-row>
      <!--表单-->
      <el-table v-loading="sysLoading" :data="sysSocialList" row-key="socialId"
                @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="50" align="center"/>
<!--        <el-table-column  label="发布内容" align="center" key="content" prop="content"/>-->
        ...
        >
          <template slot-scope="scope">
            ...
            <el-button
              ...
              type="text"
              icon="el-icon-edit"
              @click="sysHandleUpdate(scope.row)"
              v-hasPermi="['autonomy:exposal:edit']"
            >编辑
            </el-button>
            <el-button
              size="mini"
      ...
      <!--系统信息的分页操作：querySysParams-->
      <pagination
        v-show="sysTotal  >0"
        :total="sysTotal"
        :page.sync="querySysParams.pageNum"
        :limit.sync="querySysParams.pageSize"
        ...
        <el-form ref="form" :model="form" :rules="rules" label-width="100px">
          ...
          <el-form-item label="上传图片"  label-position="left">
            <el-upload
              ...
              <img v-if="this.options.socialImg" :src="this.options.socialImg" class="avatar">
              <i slot="default" class="el-icon-plus"></i>
            </el-upload>
            <span v-show="checkPhotoExposal" style="color: #C03639">请上传图片</span>
          </el-form-item>
        </el-form>
    ...
    <div :title="infoTitle" v-show=sysCheckInfo>
      <div>
        <el-form ref="forms" :model="form" :rules="rules" label-width="100px">
          ...
          <el-row>
            <el-col :span="6" :offset="1">
              <el-form-item label="发布图片：" prop="socialUrl">
                <img v-if="this.form.socialUrl" :src="this.form.socialUrl" class="avatar">
              </el-form-item>
            </el-col>
          </el-row>
            ...
            <div @click="inputFocus" class="my-reply">
              ...
              <div class="reply-btn-box">
                <el-button
                  ...
                  @click="sendComment"
                  type="primary"
                  v-hasPermi="['autonomy:exposal:addComment']"
                >发表评论
                </el-button
                >
              </div>
                ...
                >
                  ...
                  <span class="author-time">{{ item.createTime }}
                      <el-button
                        ...
                        type="primary"
                        v-hasPermi="['autonomy:social:addComment']"
                      >撤回</el-button>
                    </span>
                  </div>
...
import {
  addSystemSocial,
  delMessageSocial, delSystemSocial,
  getSocialList,
  getSystemList, querySocialById, updateSysSocial,
  uploadPicture
} from "@/api/govern/autonomy/exposal/exposal";
import {addComment, getCommentList, queryLikeAmount,revokeMsg} from "@/api/govern/autonomy/exposal/exposalComment";

export default {
  name: "lease",
  data() {
    return {
      checkPhotoExposal:false,
      communityNameList: [],
      //↓↓↓下面的是测试↓↓↓
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      getSocialList(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    queryInfo(row) {
      ...
      querySocialById(socialId).then(response => {
        ...
        this.options.socialImg = this.form.socialUrl;
        this.form.communityIds = response.commIds;
        this.communityNameList = this.letters(this.form.communityIds);
    ...
    /** 添加 修改 按钮 */
    submitForm: function () {
      if(this.form.socialUrl===undefined){
        this.checkPhotoExposal=true;
        return;
      }
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.socialId != undefined) {
            updateSysSocial(this.form).then(response => {
            ...
            addSystemSocial(this.form).then(response => {
    ...
    handleUploadPhoto(res, file) {
      ...
      uploadPicture(formData).then(response => {
        this.form.socialUrl = response.imgPath;
        this.options.socialImg = response.imgPath;
        this.$modal.msgSuccess("上传成功");
        this.checkPhotoExposal=false;
      });
    },
