...
export default {
  ...
  methods: {
    ...
    getCookie() {
      ...
      const password = Cookies.get("adminPassword");
      const userType = Cookies.get("adminUserType");
      const rememberMe = Cookies.get('adminRememberMe')
      this.loginForm = {
      ...
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          ...
          if (this.loginForm.rememberMe) {
            Cookies.set("adminUserName", this.loginForm.username, { expires: 30 });
            Cookies.set("adminPassword", encrypt(this.loginForm.password), { expires: 30 });
            Cookies.set("adminUserType", this.loginForm.userType, { expires: 30 });
            Cookies.set('adminRememberMe', this.loginForm.rememberMe, { expires: 30 });
          } else {
            Cookies.remove("adminUserName");
            Cookies.remove("adminPassword");
            Cookies.remove("adminUserType");
            Cookies.remove('adminRememberMe');
          }

          this.$store.dispatch("SysLogin", this.loginForm).then(() => {
