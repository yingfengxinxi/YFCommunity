  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="状态" align="center" prop="tourStatus">
        <template slot-scope="scope">
          <span
            :class="{
              statusGreen: scope.row.tourStatus === '0',
              statusRed: scope.row.tourStatus === '1',
            }"
          >
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="巡检类型" prop="tourName">
          <el-input v-model="form.tourName" placeholder="请输入巡检类型" />
        </el-form-item>
        <el-form-item label="状态" prop="tourStatus">
...
<script>
import * as tourType from "@/api/estate/work/inspect/type";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      tourType.getList(this.queryParams).then((response) => {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm("是否确认删除所选巡检类型?(删除后此类型无法查询)")
        .then(function () {
