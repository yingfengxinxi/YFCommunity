  ...
  <div class="app-container">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="20" :xs="24">
          <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="82px">
            <el-form-item label="巡查点名称" prop="inspectionName">
              <el-input
            ...
            <el-form-item label="巡查点状态" prop="inspectionStatus">
              <el-select
                v-model="queryParams.inspectionStatus"
                placeholder="请选择巡查点状态"
                clearable
                style="width: 240px"
              >
                <el-option
            ...
            <el-form-item label="巡查时间" prop="dateRange">
                <el-date-picker
                  v-model="dateRange"
                  size="small"
                  style="width: 240px"
          ...
          <el-table v-loading="loading" :data="inspectionList">
            ...
            <el-table-column label="巡查点名称" align="center" key="inspectionName" prop="inspectionName"/>
            <el-table-column label="巡查人" align="center" key="name" prop="name"/>
            <el-table-column label="巡查/执行状态" align="center" key="inspectionStatus" prop="inspectionStatus">
              <template slot-scope="scope">
                  {{ getInspectionStatusName(scope.row.inspectionStatus) }}
              </template>
            </el-table-column>
            <el-table-column label="巡查时间" align="center" key="createTime" prop="createTime"/>
    ...
    <div v-show="detailShow">
      <el-form ref="detailForm" :model="detailForm" label-width="140px">
        ...
        <el-steps :active=active class="stepsStyle" v-if="detailForm.inspectionStatus=='4'" > 
          <!-- v-for="(item) in caseHistoryData" :key="item.id" -->
          <el-step v-for="(item) in inspectionStatusList1" :key="item.dictValue" :title="item.dictLabel">
        ...
        <el-steps :active=active class="stepsStyle" v-else > 
          <!-- v-for="(item) in caseHistoryData" :key="item.id" -->
          <el-step v-for="(item) in inspectionStatusList2" :key="item.dictValue" :title="item.dictLabel">
            <template slot="description">
              <div class="putOnRecord_p">
                    <!-- <p v-if="item.dictValue=='0'" style="color: rgb(0, 0, 0);padding-bottom: 5px;color: #6290ff;">
                      {{ detailForm.createTime }}
        ...
        <h4 class="form-header">巡查详情</h4>
        <el-row>
          <el-col :span="7" :offset="1">
            <el-form-item label="巡查点名称:">{{ detailForm.inspectionName }}</el-form-item>
          </el-col>
          <el-col :span="10" :offset="2">
            <el-form-item label="巡查点地址:">{{ detailForm.inspectionAddress }}</el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="10" :offset="1">
            <el-form-item label="巡查情况:">{{ detailForm.inspectionSituation }}</el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="10" :offset="2">
            <el-form-item label="现场图片:">
              <image-preview v-for="(image,index) in detailForm.inspectionImages" :key="index" :src="image" :width="50" :height="50" style="margin: 0 5px;"/>
            </el-form-item>
          </el-col>
        </el-row>
...
import {
  listInspection,
  getInspectionDetail
} from "@/api/estate/customer/inspection/inspection";
...
export default {
  ...
  data() {
    return {
      ...
      inspectionStatusList1: [],
      inspectionStatusList2:[],
      dateRange: [],
      executorByOptions: [],
      //详情页是否展示
      detailShow: false,
  ...
  created() {
    ...
    this.getDicts("inspection_status").then((response) => {
      this.inspectionStatusOptions = response.data;
    });
  ...
  methods: {
    ...
    getInspectionStatusName(val) { 
      if (val) { 
        var obj = this.inspectionStatusOptions.filter(item => item.dictValue == val)[0];
        if (obj) { 
          return obj.dictLabel;
        }
      }
    ...
    handleExport() { 
      this.download('estate/inspection/export', {
        ...this.queryParams
      }, this.communityName+`小区巡查工单_${new Date().getTime()}.xlsx`)
    },
    ...
    handleDetail(row) {
      ...
      getInspectionDetail(id).then(response => {
        this.detailForm = response.data;
        //超期
        this.inspectionStatusList1 = [];
        this.inspectionStatusList2 = [];
        this.inspectionStatusOptions.forEach(item => {
          if (item.dictValue != '3') { 
            this.inspectionStatusList1.push(item);
          }
          if (item.dictValue != '4') { 
            this.inspectionStatusList2.push(item);
          }
        }); 
        this.active = (parseInt(this.detailForm.inspectionStatus)+1);
        console.log("this.active>>", this.active);
        console.log("this.inspectionStatusList1>>", this.inspectionStatusList1);
        console.log("this.inspectionStatusList2>>", this.inspectionStatusList2);
      });
      this.openView = false
