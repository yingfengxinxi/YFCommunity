  ...
  <div class="app-container">
    ...
    <div v-show=checkView >
      ...
      <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
        ...
        <el-form-item label="特色板块类型" prop="featuredSectionTypeId">
          <el-select v-model="queryParams.featuredSectionTypeId"  placeholder="请选择特色板块类型" style="width: 100%">
            <el-option
              v-for="item in featuredSectionTypeOptions"
              :key="item.dictValue"
      ...
      <!--表单-->
      <el-table v-loading="loading" :data="socialList" row-key="featuredId"  @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="50" align="center" />
        <el-table-column label="特色板块类型" align="center" key="featuredSectionTypeId" prop="featuredSectionTypeId" :formatter="fst"/>
    ...
    <el-dialog :title="title" :visible.sync="checkAddUpdateView" width="50%" append-to-body>
        <el-form ref="form" :model="form" :rules="rules" label-width="110px">
          ...
          <el-form-item label="特色板块类型" prop="featuredSectionTypeId">
            <el-select v-model="form.featuredSectionTypeId"  placeholder="请选择特色板块类型" style="width: 70%">
            <el-option
              v-for="item in featuredSectionTypeOptions"
              :key="item.dictValue"
              :label="item.dictLabel"
          ...
          <el-form-item prop="featuredUrl" label="上传图片"  label-position="left">
            <image-upload  v-model="form.featuredUrl" :limit="9"/>
          </el-form-item>
        </el-form>
    ...
    <div :title="infoTitle" v-show=sysCheckInfo>
      ...
      <div>
        <el-form ref="forms" :model="form" label-width="100px">
          ...
          <el-row>
            <el-col :offset="1">
              <el-form-item label="发布图片：" prop="featuredUrl">
                <image-preview v-for="image in form.featuredUrls" :key="image" :src="image" :width="100" :height="100" style="margin: 0px 10px;"/>
              </el-form-item>
            </el-col>
          </el-row>
...
import {
  addFeaturedSection,
  delMessageFeaturedSection,
  delSystemFeaturedSection,
  getFeaturedSectionList,
  getSystemList,
  queryFeaturedSectionById,
  updateFeaturedSection
} from "@/api/estate/featured/featuredSection";
import { addComment, getCommentList, queryLikeAmount, revokeMsg } from "@/api/estate/featured/comment";
import {
  getFeaturedSectionTypeList
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        featuredSectionTypeId: [
          {required: true, message: "请选择特色板块类型", trigger: "blur"},
        ],
        content: [
  ...
  created() {
    this.featuredSectionTypeOptions =[];
    getFeaturedSectionTypeList().then(res => { 
      res.data.forEach(item => {
        this.featuredSectionTypeOptions.push({ "dictValue": item.id, "dictLabel": item.name });
      });
  ...
  methods: {
    ...
    revokeMsgHandle(i){
      ...
      revokeMsg( this.revokeMsgForm).then(response => {
        //查询当前评论列表
        this.getComments(this.featuredId);
    ...
    fst(row, column) {
      return this.selectDictLabel(this.featuredSectionTypeOptions, row.featuredSectionTypeId)
    },
    ...
    handlerAddComment(val){
      ...
      if(this.commentIf===true){
        ...
        this.addCommentForm.featuredId=this.featuredId;
    ...
    getList() {
      ...
      getFeaturedSectionList(this.queryParams).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const featuredId = row.featuredId || this.ids;
      this.$confirm('是否确认删除所选中的的数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        return delMessageFeaturedSection(featuredId);
      }).then(() => {
        this.getList();
    ...
    /** 删除按钮操作 单个删除 */
    handleDeleteOne(row) {
      const featuredId = row.featuredId;
      this.$confirm('是否确认删除所选中的的数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
    ...
    sysHandleDelete(row) {
      ...
      this.$confirm('是否确认删除所选中的的数据项?', "警告", {
        ...
        return delSystemFeaturedSection(featuredId);
      }).then(() => {
        this.getSysList();
        this.$modal.msgSuccess("删除成功");
      })
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.featuredId)
    ...
    /* handleUpdate(row) {
       ...
       this.reset();
       const featuredId = row.featuredId;
       getStatuteById(featuredId).then(response => {
         this.checkAddUpdateView = true;
         this.form = response.data;
         this.options.socialImg= this.form.featuredUrl;
    ...
    sysHandleUpdate(row) {
      ...
      this.reset();
      const featuredId = row.featuredId;
      queryFeaturedSectionById(featuredId).then(response => {
        this.checkAddUpdateView = true;
        this.form = response.data;
        this.options.socialImg= this.form.featuredUrl;
        this.form.communityIds=response.commIds;
    ...
    queryInfo(row){
      ...
      this.getLikeNum(featuredId);
      this.getComments(featuredId);
      queryFeaturedSectionById(featuredId).then(response => {
        this.checkSysView=false;
        this.checkView=false;
    ...
    //查询点赞数量
    getLikeNum(featuredId){
      queryLikeAmount(featuredId).then(response => {
        this.likeAmount=response.likeAmount;
      });
    },

    //获取评论列表
    getComments(featuredId){
      getCommentList(featuredId).then(response => {
    ...
    submitForm: function() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if(this.form.featuredId != undefined){
            updateFeaturedSection(this.form).then(response => {
            ...
            addFeaturedSection(this.form).then(response => {
