<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="类型" align="center" prop="propertyType">
        <template slot-scope="scope">
          {{ scope.row.propertyType=='0'?'固定资产':"消耗资产" }}
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      ...
      >
      <el-form-item label="类型" prop="propertyType">
        <el-select v-model="form.propertyType" placeholder="请选择类型" clearable style="width: 100%;">
                <el-option
                  v-for="(item,index) in propertyTypeOptions"
                  :key="index"
                  :value="item.id"
...
import {
  ...
  del,
  getById
} from "@/api/estate/property/propertyType";

export default {
  name: "propertyType",
  data() {
    return {
      propertyTypeOptions: [{ "id": "0", "name": "固定资产" },{"id":"1","name":"消耗资产"}],
      // 遮罩层
      loading: true,
      keyUrl: "",
      // 选中数组
