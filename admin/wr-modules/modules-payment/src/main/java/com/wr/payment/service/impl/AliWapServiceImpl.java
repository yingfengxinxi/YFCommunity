 ...
 */
@Service
public class AliWapServiceImpl implements AliPayService {
    private static final Logger log = LoggerFactory.getLogger(AliWapServiceImpl.class);

    @Override
    public Boolean support(String method) {

        return PayConstants.WAP.equals(method);
    }
    ...
    public AjaxResult execute(AliPay aliPay) {
        try {
            //创建订单
            AlipayTradeWapPayResponse response = AliPayUtil.tradeWapPay(aliPay);
