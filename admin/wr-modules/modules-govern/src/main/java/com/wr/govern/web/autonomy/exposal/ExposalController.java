 ...
 */
@RestController
@RequestMapping("/exposal")
public class ExposalController extends BaseController {


    @Autowired
    private ExposalService exposalService;
    @Autowired
   ...
   @RequiresPermissions("autonomy:exposal:list")
    public TableDataInfo messageList(Social social) throws ParseException {
        startPage();
        List<Social> list = exposalService.getList(social);
    ...
    public TableDataInfo systemList(Social social) throws ParseException {
        startPage();
        List<Social> list = exposalService.systemList(social);
        return getDataTable(list);
    }
     ...
     */
    @PostMapping
    @RequiresPermissions("autonomy:exposal:add")
    ...
    public AjaxResult addSystemSocial(@Validated @RequestBody Social social) {
        ...
        Integer inte = exposalService.addSystemSocial(social);
    ...
    public AjaxResult delSystemSocial(@PathVariable Long[] id){
        return toAjax(exposalService.delSystemSocial(id));
    }
     ...
     * @return
     */
    @RequiresPermissions("autonomy:exposal:edit")
    @Log(title = "修改", businessType = BusinessType.UPDATE)
    @PutMapping
    public AjaxResult updateSysSocial(@Validated @RequestBody Social social){
        int i= exposalService.updateSysSocial(social);
        return toAjax(i);
    }
     ...
     * @return
     */
    @RequiresPermissions("autonomy:exposal:query")
    ...
    public AjaxResult getInfo(@PathVariable(value = "socialId", required = false) Long socialId) {
        ...
        if (StringUtils.isNotNull(socialId)){
            //2.查询social表格中信息
            Social social = exposalService.getSocialById(socialId);
            //3.查询小区与社交表的关联表
            List<Long>  ids =  exposalService.querySocialRangById(socialId);
