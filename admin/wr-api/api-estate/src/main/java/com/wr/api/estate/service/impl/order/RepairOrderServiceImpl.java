...
import com.wr.api.estate.mapper.order.RepairOrderMapper;
...
public class RepairOrderServiceImpl implements OrderService {
    ...
    public List<OrderListVO> listPage(OrderQueryVO query) {
        ...
        switch (query.getOrderType()) {
            case 5:
                list = repairOrderMapper.listHomeOrders(query);
                break;
            case 7:
                list = repairOrderMapper.listParkOrders(query);
                break;
            case 8:
                list = repairOrderMapper.listDisasterOrders(query);
                break;
            default:
                break;
        }
    ...
    public Object detail(OrderQueryVO query) {
        RepairOrderDetailVO detail;
        switch (query.getOrderType()) {
            // 居家维修工单
            case 5:
                detail = repairOrderMapper.detailHomeOrder(query.getOrderId());
                break;
            // 园区报事工单
            case 7:
                detail = repairOrderMapper.detailParkOrder(query.getOrderId());
                break;
            // 灾害报事工单
            case 8:
                detail = repairOrderMapper.detailDisasterOrder(query.getOrderId());
                break;
            default:
                detail = new RepairOrderDetailVO();
                break;
        }
    ...
    public void robOrder(OrderQueryVO query) {
        int i = repairOrderMapper.robOrder(query);
        if (i <= 0) {
    ...
    public void receiveOrder(OrderQueryVO query) {
        int i = repairOrderMapper.receiveOrder(query);
        if (i <= 0) {
    ...
    public void startPerform(OrderQueryVO order) {
        int i = repairOrderMapper.startPerform(order);
        if (i <= 0) {
    ...
    public void finish(OrderQueryVO order) {
        repairOrderMapper.finish(order);
    ...
    public void transfer(OrderQueryVO order) {
        ...
        int i = repairOrderMapper.transfer(order);
        if (i <= 0) {
