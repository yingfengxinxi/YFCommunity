...
import com.wr.api.estate.mapper.order.CleanOrderMapper;
...
public class CleanOrderServiceImpl implements OrderService {
    ...
    public List<OrderListVO> listPage(OrderQueryVO query) {
        return cleanOrderMapper.listCleanOrders(query);
    }
     ...
     * @author yx
     * @date 2022/11/15 11:49
     */
    @Override
    public Object detail(OrderQueryVO query) {
        CleanOrderDetailVO detail = cleanOrderMapper.detailCleanOrder(query.getOrderId());
    ...
    public void robOrder(OrderQueryVO query) {
        int i = cleanOrderMapper.robOrder(query);
        if (i <= 0) {
    ...
    public void receiveOrder(OrderQueryVO query) {
        int i = cleanOrderMapper.receiveOrder(query);
        if (i <= 0) {
    ...
    public void startPerform(OrderQueryVO order) {
        int i = cleanOrderMapper.startPerform(order);
        if (i <= 0) {
    ...
    public void transfer(OrderQueryVO order) {
        ...
        // 转让
        int i = cleanOrderMapper.transfer(order);
        if (i <= 0) {
