...
import com.wr.api.owner.service.estateapi.featured.FeaturedSectionTypeService;
 ...
 */
@RestController
@RequestMapping("/estateapi/featuredSectionType")
@Api(tags = "业主端-特色板块类型")
public class FeaturedSectionTypeController extends BaseController {
    ...
    @GetMapping("/list")
    @ApiOperation("特色板块列表")
    public AjaxResult list(@RequestParam Map map) {
        LambdaQueryWrapper<FeaturedSectionType> queryWrapper = new LambdaQueryWrapper<>();

        queryWrapper.eq(FeaturedSectionType::getEstateId, MapUtil.getLong( map,"estateId" ));
